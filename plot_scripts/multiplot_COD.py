...
COD_diff = COD_CMIP6_model_mean - COD_CMIP5_model_mean
...
fig, axes = plt.subplots( ncols=3, nrows=1, figsize=(10, 12), subplot_kw={'projection': proj})
...
#CMIP5 model mean
cont = ax[0].pcolormesh(ds['LON'], ds['LAT'], COD_CMIP5_model_mean,   
                                    transform=ccrs.PlateCarree(),
                                    cmap='RdBu_r', vmin= -0.1, vmax=0.1)
#CMIP6 model mean
cont1 = ax[1].pcolormesh(ds['LON'], ds['LAT'], COD_CMIP6_model_mean,   
                                    transform=ccrs.PlateCarree(),
                                    cmap='RdBu_r', vmin= -0.1, vmax=0.1)
#CMIP6 model mean
cont2 = ax[2].pcolormesh(ds['LON'], ds['LAT'], COD_diff,   
                                    transform=ccrs.PlateCarree(),
                                    cmap='RdBu_r', vmin= -0.1, vmax=0.1)
...
cbar.set_label('('+season+') Cloud Optical Depth amonalies [$\%$]', fontsize=14)
...
fig.savefig('str(TAS)+'_'+season+'_deg_COD_multiplot.pdf',bbox_inches='tight',dpi=100)
